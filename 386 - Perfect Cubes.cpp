...
unsigned long long int cube(unsigned long long int n){
    return n * n * n;
}
...
int main(){
    ...
    unsigned long long int a, b, c, d, A, B, C, D;
    ...
    for (a = 6; a <= 200; ++a){
        ...
        for (b = 2; b < a; ++b){
            ...
            for (c = b; c <= a; ++c){
                ...
                for (d = c; d <= a; ++d){
                    D = cube(d);
                    if (A == B + C + D) cout << "\"Cube = " << a << ", Triple = (" << b << "," << c << "," << d << ")\\n\"" << endl;
                }
            }
        }
    }
